...
import { sportCreateSchema, type SportCreateFormValues } from '../../schemas/sportsSchema';

interface SportsFormProps {
  onSubmit: (data: SportCreateFormValues) => void;
  onCancel: () => void;
  isSubmitting?: boolean;
}

export const SportsForm: React.FC<SportsFormProps> = ({ onSubmit, onCancel, isSubmitting = false }) => {
  const {
    register,
    control,
    handleSubmit,
    formState: { errors },
  } = useForm<SportCreateFormValues>({
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-6">
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-gray-900">Bộ quy tắc & Thông số</h3>
          ...
          <div className="space-y-3">
            {fields.length > 0 && (
              <div className="flex gap-3 px-1 text-xs font-medium text-gray-500 uppercase tracking-wider">
                <div className="w-[35%]">Mã quy tắc (Key) <span className="text-red-500">*</span></div>
                <div className="w-[20%]">Giá trị <span className="text-red-500">*</span></div>
            ...
            {fields.length === 0 && (
              <div className="text-center py-6 border-2 border-dashed border-gray-200 rounded-lg bg-gray-50">
                <p className="text-sm text-gray-500">Chưa có quy tắc nào được thiết lập.</p>
              </div>
            )}
